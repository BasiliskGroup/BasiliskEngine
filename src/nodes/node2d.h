...
class Node2D : public VirtualNode<Node2D, vec2, float, vec2> {
    ...
    public:
        Node2D(Scene2D* scene, Mesh* mesh, Texture* texture, vec2 position={0, 0}, float rotation=0, vec2 scale={1, 1});
        Node2D(Node2D* parent, Mesh* mesh, Texture* texture, vec2 position={0, 0}, float rotation=0, vec2 scale={1, 1});
