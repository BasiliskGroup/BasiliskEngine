...
bool RenderBuffer::initialize(const char* vertexShaderPath, const char* fragmentShaderPath) {
    if (initialized) {
...
void RenderBuffer::initializeCompute() {
...
void RenderBuffer::markChunkAndNeighborsDirty(int cx, int cy) {
...
void RenderBuffer::markChunkDirty(int px, int py) {
...
void RenderBuffer::setBackPixel(int x, int y, const Color& color) {
    if (x >= 0 && x < width && y >= 0 && y < height)
        getBackBuffer()[y * width + x] = color;
}
Color RenderBuffer::getActivePixel(int x, int y) const {
    if (x >= 0 && x < width && y >= 0 && y < height)
        return getActiveBuffer()[y * width + x];
    return Color::Empty();
}
Color RenderBuffer::getBackPixel(int x, int y) const {
    if (x >= 0 && x < width && y >= 0 && y < height)
...
void RenderBuffer::applyBrush(int pixelX, int pixelY, int radius, const Color& color) {
...
void RenderBuffer::applyParticleBrush(int pixelX, int pixelY, int radius, uint32_t spawnCount, const Color& color) {
...
std::string RenderBuffer::loadShaderSource(const char* filepath) {
...
const std::vector<Color>& RenderBuffer::getActiveBuffer() const {
    return firstActive ? buffers.first : buffers.second;
}
const std::vector<Color>& RenderBuffer::getBackBuffer() const {
