    ...
    while (engine->isRunning()) {
        ...
        for (bsk::Force* force = scene->getSolver()->getForces(); force != nullptr; force = force->getNext()) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                glm::vec2 r = manifold->isA() ? manifold->getRAW()[i] : manifold->getRBW()[i];
                r += pos;

                bsk::Node2D* node = new bsk::Node2D(scene, { .mesh=quad, .material= manifold->isA() ? material2 : material3, .position=r, .scale={0.125, 0.125} });
                node->setLayer(0.9);
